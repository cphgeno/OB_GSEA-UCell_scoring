# define UCell wrapper function
UCell_function <- function(ranks_data, metadata, genesets, analysis_name, output_folder, input_type, tool_colour){
    ...
    #UCell receives the ranks as a matrix
    ranks_data_matrix <- ranks_data_ordered %>%
        mutate(across(-Geneid, as.numeric)) %>% 
        column_to_rownames("Geneid") %>% 
        as.matrix()
    
    UCell_matrix <- ScoreSignatures_UCell(ranks_data_matrix,
                                            features = genesets,
                                            name = "", # no suffix to geneset names
                                            maxRank = nrow(ranks_data_matrix))
    UCell_score <- as.data.frame(t(as.matrix(UCell_matrix)))
    
    UCell_score <- UCell_score[order(rownames(UCell_score)), , drop = FALSE]
    write.table(rownames_to_column(UCell_score, var = 'GOI_Set'),
                file.path(output_folder, paste0(analysis_name, "-fullNES.tsv")),
                sep = '\t', quote = FALSE, row.names = FALSE)

    plot_GSEApheatmap_wNAs(UCell_score,
        file.path(output_folder, 'ES_plots', paste0(analysis_name, '.png')),
        paste0(analysis_name, " - ucell_", input_type),
    ...
    for (annotation_group in annotation_list){
        ...
        UCell_score_annot <- UCell_score[, colnames(UCell_score) %in% metadata_of_annotation$filename, drop = FALSE]
        
        write.table(rownames_to_column(UCell_score_annot, var = 'GOI_Set'),
                    file.path(output_folder, 'ES_matrix', paste0(annotation_group, ".tsv")),
                    sep = '\t', quote = FALSE, row.names = FALSE)
        
        plot_GSEApheatmap_wNAs(UCell_score_annot,
            file.path(output_folder, 'ES_plots', paste0(annotation_group, '.png')),
            paste0(annotation_group_og, " - ucell_", input_type),
            tool_colour)
    }
}
