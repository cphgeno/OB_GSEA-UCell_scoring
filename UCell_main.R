...
source(file.path(get_script_dir(), "UCell_helper_functions.R"))
...
TOOL_COLOURS <- list(
  ucell_DeltaCentroid = "#8B564A",
  ucell_RankExpr = "#C39C94"
)

# check analysis type to fetch correct input
if (opts$input_type == "RankExpr"){
  ranks_df <- opts$ranks_df_sample
  tool_colour = TOOL_COLOURS[[paste0("ucell_", opts$input_type)]]
} else {
...
UCell_function(input_df, metadata_df, genesets_list, opts$name, opts$output_dir, opts$input_type, tool_colour)
